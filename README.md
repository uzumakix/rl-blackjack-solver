...
This project served as my foundational sandbox for modeling stochastic environments and episodic tasks, directly preceding the more complex imperfect-information game tree built in the CFR+ Poker Solver.

# Monte Carlo Reinforcement Learning for Optimal Blackjack Strategy

A from-scratch implementation of **Monte Carlo Exploring Starts** (Sutton & Barto 2018, Section 5.3) applied to casino Blackjack. The agent learns an optimal policy over 5 million episodes by iteratively updating action-value estimates Q(s,a) through first-visit returns, converging to a strategy that matches known basic strategy tables.
